...
export default function ChatScreen({ onBack, onNavigateToSettings, onNavigateToCreateRoom, chatRoomState, onLeaveChatRoom }: ChatScreenProps) {
  ...
  const [hasChatRoom, setHasChatRoom] = useState<Boolean | null>(false);
...
const getMyChatInfo = async () => {
  ...
  useEffect(() => {
    if (!chatRoomState.hasRoom || !chatRoomState.roomId) {
    ...
    const client = new Client({
      ...
      onConnect: () => {
        ...
        client.subscribe(`/topic/rooms/${chatRoomState.roomId}`, (frame) => {
          try {
        ...
        // 초기 데이터 로드
        getAllChatMessages(chatRoomInfo?.id as number);
        getMyChatInfo();
        getUserInfo();
      },
  ...
  }, [chatRoomState.hasRoom, chatRoomState.roomId]);
