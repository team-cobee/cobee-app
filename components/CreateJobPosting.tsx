...
//   onBack: () => void;
//   onSuccess: () => void;
//   onComplete: (jobId: string) => void; // 백엔드 postId를 문자열로 넘겨줌
//   editJobId?: string | null;
// }
...
// const toSmokingEnum = (v: string) =>
//   v === 'no' ? 'NOT_SMOKE' : v === 'yes' ? 'SMOKE' : 'NONE';

// const toSnoringEnum = (v: string) =>
//   v === 'no' ? 'NO_SNORE' : v === 'yes' ? 'SNORE' : 'NONE';
...
//   selectButtonSelected: { borderColor: '#F7B32B', backgroundColor: '#fef3c7' },
//   selectButtonSelectedText: { color: '#374151' },
...
//   },
//   chipActive: { borderColor: '#F7B32B', backgroundColor: '#fef3c7' },
...
//     ageMin: 18,              // → minAge
//     ageMax: 100,             // → maxAge
//     lifestyle: '',           // 'morning' | 'evening' | 'none' : LifeStyle Enum
//     personality: '',         // 'introvert' | 'extrovert' | 'none' | : Personality Enum
//     smoking: '',             // 'impossible' | 'none'
//     snoring: '',             // 'impossible' | 'none'
//     pets: '',                // 'possible' | 'impossible' | 'none' | 

//     // Step 3
//     hasRoom: '',             // 'true' | 'false' : Boolean

//     // Step 4 (주소/설명/좌표)
//     address: '',
//     latitude: '',            // 문자열 입력 → 제출시 float 변환
//     longitude: '',
//     images: [] as string[], 
...
//         return formData.title.trim().length > 0 && formData.recruitCount > 0;
//       case 2: {
//         const ageOk = Number.isFinite(formData.ageMin) && Number.isFinite(formData.ageMax) && formData.ageMin <= formData.ageMax;
//         return (
//           ageOk &&
//           !!formData.lifestyle &&
//           !!formData.personality &&
...
//         return !!formData.hasRoom;
//       case 4:
//         // 방이 있으면 주소 필수, 좌표는 선택값
//         return formData.hasRoom === 'none' || formData.address.trim().length > 0;
//       case 5:
//         return true; // 이미지 URL은 선택
//       default:
//         return true;
//     }
//   };

//   /** 제출 → 백엔드 /recruits POST */
//   const handleSubmit = async () => {
//     try {
//       const body = {
//         title: formData.title,
//         recruitCount: formData.recruitCount,
//         rentCostMin: Number(formData.depositMin),
//         rentCostMax: Number(formData.depositMax),
//         monthlyCostMin: Number(formData.monthlyRentMin),
//         monthlyCostMax: Number(formData.monthlyRentMax),
...
//         lifestyle: toLifestyleEnum(formData.lifestyle),
//         personality: toPersonalityEnum(formData.personality),
...
//       const res = await fetch('http://localhost:8080/recruits', {
//         method: 'POST',
//         headers: { 'Content-Type': 'application/json' },
//         body: JSON.stringify(body),
//         // 인증이 필요하면 credentials/Authorization 헤더 추가
//       });
...
//       const json = await res.json();
//       // ApiResponse 래퍼 고려: { data: RecruitResponse, ... } 또는 그냥 RecruitResponse
...
//               value={String(formData.monthlyRentMin)}
//               onChangeText={(text) =>
//                 setFormData({ ...formData, monthlyRentMin: parseInt(text) || 0 })
//               }
//               keyboardType="numeric"
//             />
//             <Text style={styles.rangeLabel}>~</Text>
...
//               value={String(formData.monthlyRentMax)}
//               onChangeText={(text) =>
//                 setFormData({ ...formData, monthlyRentMax: parseInt(text) || 0 })
//               }
//               keyboardType="numeric"
//             />
//           </View>
...
//           <View style={styles.optionsContainer}>
//             {[
//               { value: 'no', label: '비흡연자 선호' },
//               { value: 'yes', label: '흡연자도 가능' },
...
//           <View style={styles.optionsContainer}>
//             {[
//               { value: 'no', label: '코골이 X 선호' },
//               { value: 'yes', label: '코골이도 가능' },
//               { value: 'any', label: '상관없음' },
//             ].map((o) => (
//               <TouchableOpacity
//                 key={o.value}
...
//             placeholder="추가로 알려주고 싶은 내용을 자유롭게 작성해주세요"
//             value={formData.detailDescription}
//             onChangeText={(text) => setFormData({ ...formData, detailDescription: text })}
//             multiline
//             style={{ height: 100, textAlignVertical: 'top' }}
//           />
//         </View>
...
//             placeholder="선택 입력"
//             value={formData.additionalDescription}
//             onChangeText={(text) => setFormData({ ...formData, additionalDescription: text })}
//             multiline
//             style={{ height: 80, textAlignVertical: 'top' }}
//           />
//         </View>
